...
export default function Sidebar({ isOpen, onClose }) {
  ...
  return (
    <aside
      className={`sidebar fixed left-0 top-0 h-screen w-64 flex flex-col z-50 transform transition-transform duration-300 ease-in-out
    ...
    >
      ...
      <div className="logo-container border-b border-red-700 py-4">
        <img
          ...
          className="max-w-[140px] md:max-w-[160px] h-auto mx-auto"
          data-testid="logo"
        />
      </div>

      {/* Navigation */}
      <nav className="flex-1 p-3 md:p-4 space-y-1 overflow-y-auto" data-testid="sidebar-nav">
        {navItems.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
      ...
      {/* Quick Action */}
      <div className="p-3 md:p-4 border-t border-red-700">
        <NavLink
          to="/invoices/create"
          onClick={handleNavClick}
      ...
      {/* Footer */}
      <div className="p-3 md:p-4 text-xs text-white/70 text-center">
